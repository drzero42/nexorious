...
// Test admin credentials - only used in this setup file
const TEST_ADMIN = {
  username: 'e2e-admin',
...
setup('authenticate as admin', async ({ page }) => {
  ...
  // Verify admin is properly authenticated by checking for username on page
  // The username can appear as text or in a button - check both
  const usernameButton = page.getByRole('button', { name: TEST_ADMIN.username });
  const usernameText = page.getByText(TEST_ADMIN.username);

  try {
    // First try to find username as a button (common in dropdown menus)
    await expect(usernameButton).toBeVisible({ timeout: 5000 });
  } catch {
    // If not found as button, look for it as text anywhere on the page
