 ...
 <div class="flex flex-wrap gap-2 {compact ? 'gap-1.5' : 'gap-2.5'}">
  {#each visiblePlatforms as group (group.platform.id)}
   ...
   <div class="inline-flex items-center rounded-lg transition-all duration-200 border-2 shadow-lg
               ...
               {compact ? 'px-2.5 py-1.5 min-h-[32px]' : 'px-4 py-2.5 min-h-[40px]'}
               {enableHover ? 'hover:scale-105 hover:shadow-xl cursor-help' : ''}" 
        role="status" 
   ...
   >
    ...
    <div class="flex items-center {compact ? 'gap-1.5' : 'gap-2'}">
     <span class="{compact ? 'text-sm' : 'text-base'}" role="img" aria-hidden="true">
      {platformStyle.icon}
     </span>
    ...
    {:else if group.storefronts.length > 0}
     <!-- Compact storefront count indicator -->
  ...
  {#if hiddenCount > 0}
   <div class="inline-flex items-center rounded-lg transition-all duration-200 border-2 shadow-lg
    ...
    <div class="flex items-center {compact ? 'gap-1.5' : 'gap-2'}">
     <span class="{compact ? 'text-sm' : 'text-base'}" role="img" aria-hidden="true">📦</span>
...
{:else}
 <div class="inline-flex items-center {compact ? 'px-2.5 py-1.5' : 'px-4 py-2.5'} rounded-lg bg-gray-100 border border-gray-200">
