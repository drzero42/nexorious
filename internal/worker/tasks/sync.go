...
type dispatchSyncPayload struct {
	JobID      string `json:"job_id"`
	UserID     string `json:"user_id"`
	Storefront string `json:"storefront"`
}

// NewDispatchSyncHandler returns a worker task handler that:
...
// 6. Dispatches process_sync_item tasks for each non-skipped game
...
func NewDispatchSyncHandler(db *bun.DB, steam SteamLibraryAdapter, psnLib PSNLibraryAdapter) func(ctx context.Context, task *models.PendingTask) error {
	return func(ctx context.Context, task *models.PendingTask) error {
		...
		if err := db.NewSelect().Model(&cfg).
			Where("user_id = ? AND storefront = ?", p.UserID, p.Storefront).
			Scan(ctx); err != nil {
			failSyncJob(ctx, db, p.JobID, "no sync config found")
			return nil
		}
		if cfg.StorefrontCredentials == nil {
			failSyncJob(ctx, db, p.JobID, "credentials not configured")
			return nil
		}
		...
		case "steam":
			...
			if err := json.Unmarshal([]byte(*cfg.StorefrontCredentials), &creds); err != nil {
				failSyncJob(ctx, db, p.JobID, "invalid steam credentials")
				return nil
			}
			raw, err := steam.GetOwnedGames(ctx, creds.WebAPIKey, creds.SteamID)
			if err != nil {
				failSyncJob(ctx, db, p.JobID, fmt.Sprintf("fetch steam library: %v", err))
				return nil
			}
			for _, e := range raw {
		...
		case "psn":
			...
			if err := json.Unmarshal([]byte(*cfg.StorefrontCredentials), &creds); err != nil {
				failSyncJob(ctx, db, p.JobID, "invalid psn credentials")
				return nil
			}
			if !creds.IsVerified {
				failSyncJob(ctx, db, p.JobID, "psn_token_expired")
				return nil
			}
			raw, err := psnLib.GetLibrary(ctx, creds.NpssoToken)
			if err != nil {
		...
		default:
			failSyncJob(ctx, db, p.JobID, fmt.Sprintf("unknown storefront: %s", p.Storefront))
			return nil
		}
		...
		_ = db.NewSelect().Model(&available).
		...
		// ── 6. Dispatch process_sync_item tasks for non-skipped games ─────────
		...
		_ = db.NewSelect().Model(&toProcess).
		...
		for _, eg := range toProcess {
			...
			// Insert pending_task — priority is INTEGER (5 = high sync priority).
			_, _ = db.NewRaw(
				...
				uuid.NewString(),
				fmt.Sprintf(`{"job_item_id":%q}`, itemID),
			).Exec(ctx)
		}
...
// NewProcessSyncItemHandler returns a worker task handler that resolves a single
// sync job item: IGDB matching → user_game find-or-create → user_game_platform
// find-or-create with ownership-rank guard → marks the item completed.
func NewProcessSyncItemHandler(db *bun.DB, igdbClient *igdbsvc.Client) func(ctx context.Context, task *models.PendingTask) error {
	return func(ctx context.Context, task *models.PendingTask) error {
		// ── 1. Parse payload ──────────────────────────────────────────────────
		...
		if eg.ResolvedIGDBID == nil && igdbClient != nil && igdbClient.Configured() {
			candidates, err := igdbClient.SearchGames(ctx, eg.Title, 10)
			if err != nil {
		...
		// ── 7. Still no IGDB ID → pending_review ─────────────────────────────
		...
		// ── 8. Resolve platform and storefront slugs ──────────────────────────
		...
		if err != nil {
			...
			if ferr := db.NewRaw(
				...
				syncMarkItemFailed(ctx, db, &item, fmt.Sprintf("find/create user_game: %v", ferr))
				syncCheckJobCompletion(ctx, db, item.JobID)
				return nil
			}
		}

		// ── 10. Find or create user_game_platform ─────────────────────────────
		ownership := ""
