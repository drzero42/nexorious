...
class SteamImportService(ImportSourceService):
    ...
    async def import_library(self, user_id: str) -> ImportResult:
        ...
        try:
            ...
            steam_games_service = create_steam_games_service(self.session)
    ...
    async def list_games(self, 
        ...
        # Convert to ImportGame format
        games = []
        for result in results:
            steam_game = result[0]
            user_game_id = result[1] if len(result) > 1 else None
...
def create_steam_import_service(session: Session) -> SteamImportService:
    """Factory function to create Steam import service."""
    return SteamImportService(session)