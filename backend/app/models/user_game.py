...
class UserGame(SQLModel, table=True):
    ...
    game_id: Mapped[int] = Field(foreign_key="games.id", index=True)
    ownership_status: Mapped[OwnershipStatus] = Field(default=OwnershipStatus.OWNED)
    personal_rating: Mapped[Optional[Decimal]] = Field(default=None, max_digits=2, decimal_places=1)
    is_loved: Mapped[bool] = Field(default=False, index=True)
    play_status: Mapped[PlayStatus] = Field(default=PlayStatus.NOT_STARTED, index=True)
    hours_played: Mapped[int] = Field(default=0)
    personal_notes: Mapped[Optional[str]] = Field(default=None)
    acquired_date: Mapped[Optional[date]] = Field(default=None)
...
class UserGamePlatform(SQLModel, table=True):
    ...
    id: Mapped[str] = Field(default_factory=lambda: str(uuid.uuid4()), primary_key=True)
    user_game_id: Mapped[str] = Field(foreign_key="user_games.id", index=True)
    platform_id: Mapped[Optional[str]] = Field(default=None, foreign_key="platforms.id", index=True)
    storefront_id: Mapped[Optional[str]] = Field(default=None, foreign_key="storefronts.id")
    store_game_id: Mapped[Optional[str]] = Field(default=None, max_length=200)
    store_url: Mapped[Optional[str]] = Field(default=None, max_length=500)
    is_available: Mapped[bool] = Field(default=True)
    original_platform_name: Mapped[Optional[str]] = Field(default=None, max_length=200, description="Original platform name for unresolved platforms")
    created_at: Mapped[datetime] = Field(default_factory=lambda: datetime.now(timezone.utc))
    updated_at: Mapped[datetime] = Field(default_factory=lambda: datetime.now(timezone.utc))
