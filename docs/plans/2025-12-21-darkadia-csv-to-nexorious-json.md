# Darkadia CSV to Nexorious JSON Converter Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a standalone script that converts Darkadia CSV exports to Nexorious JSON format with interactive IGDB lookup.

**Architecture:** Single Python script with self-contained mapping dictionaries, using the existing IGDB service for rate-limited API calls. CSV parsing groups multi-row games (continuation rows), validates mappings upfront (fail fast), and prompts interactively for ambiguous IGDB matches.

**Tech Stack:** Python 3.13, existing IGDB service from backend, standard library (csv, json, argparse)

**Branch Workflow:** Create a feature branch before starting implementation. Never commit directly to main.

---

## Task 1: Create Script Skeleton with CLI

**Files:**
- Create: `backend/scripts/darkadia_to_nexorious.py`
...
git commit -m "feat: add Darkadia to Nexorious converter script skeleton"
```

---

## Task 2: Add Mapping Dictionaries

**Files:**
- Modify: `backend/scripts/darkadia_to_nexorious.py`

**Step 1: Add platform mapping dictionary**
...
**Step 2: Add storefront mapping dictionary**

Add after platform mappings:
...
```bash
git add backend/scripts/darkadia_to_nexorious.py
git commit -m "feat: add platform and storefront mapping dictionaries"
```

---

## Task 3: Add CSV Parsing and Consolidation

**Files:**
- Modify: `backend/scripts/darkadia_to_nexorious.py`

**Step 1: Add dataclasses for parsed data**
...
**Step 3: Update main to use parsing**

Update the main function to call parse_csv:
...
```bash
git add backend/scripts/darkadia_to_nexorious.py
git commit -m "feat: add CSV parsing and game consolidation logic"
```

---

## Task 4: Add Mapping Validation (Fail Fast)
...
**Step 2: Update main to validate**
...
```bash
git add backend/scripts/darkadia_to_nexorious.py
git commit -m "feat: add mapping validation with fail-fast behavior"
```

---

## Task 5: Add IGDB Integration

**Files:**
- Modify: `backend/scripts/darkadia_to_nexorious.py`

**Step 1: Add IGDB imports and setup**
...
**Step 2: Add interactive IGDB matching function**
...
**Step 3: Add batch IGDB lookup function**
...
```bash
git add backend/scripts/darkadia_to_nexorious.py
git commit -m "feat: add IGDB integration with interactive matching"
```

---

## Task 6: Add JSON Output Generation

**Files:**
- Modify: `backend/scripts/darkadia_to_nexorious.py`

**Step 1: Add play status derivation**
...
**Step 2: Add JSON generation function**
...
```bash
git add backend/scripts/darkadia_to_nexorious.py
git commit -m "feat: add JSON output generation with stats"
```
...
**Step 2: Run script with test data to verify**

Run: `cd /home/abo/workspace/home/nexorious/backend && uv run python scripts/darkadia_to_nexorious.py --help`

Expected: Help message with positional arguments shown

**Step 3: Commit**

```bash
git add backend/scripts/darkadia_to_nexorious.py
git commit -m "feat: wire up complete Darkadia to Nexorious conversion flow"
```

---

## Task 8: Test with Real Data and Iterate on Mappings

**Files:**
- Modify: `backend/scripts/darkadia_to_nexorious.py` (add missing mappings)

**Step 1: Run against real Darkadia CSV**

User provides their Darkadia CSV file. Run:

```bash
cd /home/abo/workspace/home/nexorious/backend
export IGDB_CLIENT_ID="..."
export IGDB_CLIENT_SECRET="..."
uv run python scripts/darkadia_to_nexorious.py /path/to/darkadia.csv output.json
```

**Step 2: Add missing mappings**

When script fails with unmapped platforms/storefronts, add them to the dictionaries and re-run.

**Step 3: Verify output**

Check output.json matches expected Nexorious format and can be imported.

**Step 4: Final commit**

```bash
git add backend/scripts/darkadia_to_nexorious.py
git commit -m "feat: complete Darkadia to Nexorious converter with all mappings"
```
...
| Task | Description |
|------|-------------|
| 1 | Script skeleton with CLI argument parsing |
| 2 | Platform and storefront mapping dictionaries |
| 3 | CSV parsing and multi-row game consolidation |
| 4 | Mapping validation with fail-fast behavior |
| 5 | IGDB integration with interactive matching |
| 6 | JSON output generation |
| 7 | Wire everything together |
| 8 | Test with real data and iterate on mappings |
