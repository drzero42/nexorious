...
- **`sync review <sf>`** — interactive only (errors off-TTY with a hint to use `resolve`/`skip`). Lists external games with `sync_status == "needs_review"`. For each: print title; offer **[s]earch IGDB & pick** (reuse `SearchIGDB` by the source title → numbered candidates → `RematchExternalGame(igdb_id)`), **[k]skip** (`SkipExternalGame`), **[n]ext** (leave for later), **[q]uit**. If a rematch fails (e.g. the server needs an `orphan_action` because it would orphan a user-game), the error is printed and the walker moves on — the user re-runs `sync resolve <ref> --igdb-id N --orphan-action remove` for that one. (The interactive walker deliberately does not prompt for orphan handling; that lives on the non-interactive `resolve`.)
