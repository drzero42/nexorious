...
| PSN integration | `github.com/sizovilya/go-psn-api` (NPSSO → OAuth token exchange + profile fetch) |
...
- `GET /api/status` — the Python endpoint returns `{"igdb_configured": bool}`. In the Go port, IGDB credentials are optional (see "IGDB Optional Credentials" section) and `GET /health` already reports `igdb_configured` — a separate `/api/status` endpoint is redundant and is not carried forward
