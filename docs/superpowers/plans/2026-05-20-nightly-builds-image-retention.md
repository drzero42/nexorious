...
**Architecture:** All changes happen inside `.github/workflows/build-push.yaml`. The workflow's `on:` triggers swap `push: main` for `schedule: '0 2 * * *'` + `workflow_dispatch` (release stays). The current `test` job (a `workflow_call` invocation of `test.yaml`) is deleted entirely, removing the second test run — `test.yaml` already triggers on push directly. A new `pre-check` job runs before `build-push` and outputs `should_build`; on the nightly schedule it asks the GitHub Packages REST API whether HEAD's short SHA already has a tagged image and skips the downstream chain if so. A new `cleanup` job runs in parallel with `build-push-chart` after `build-push` succeeds (non-release only) and uses `vlaurin/action-ghcr-prune@v0.6.0` to keep the 30 newest dev image versions (`nexorious`) and 30 newest dev chart versions (`charts/nexorious`), protecting semver versions and the floating `dev` / `0.0.0-dev` tags via `keep-tags-regexes`. (The original spec named `actions/delete-package-versions@v5`; that action turned out to filter on the manifest digest rather than tags, so it cannot protect by tag — see the "Deviation from spec" note in Task 3 below.)

**Tech Stack:** GitHub Actions YAML, GitHub Packages REST API (called via `gh api`), `vlaurin/action-ghcr-prune@v0.6.0`.
...
- **Auto-deletion of untagged versions** — `vlaurin/action-ghcr-prune` has a `prune-untagged: true` knob, but the spec did not ask for it and untagged manifest pruning has edge cases (e.g. multi-arch platform-specific manifests appear untagged in GHCR's UI). Left off as the safer default; revisit if untagged versions accumulate.
