...
These static-prefix routes must come before all parameterised `/:storefront*` routes. The current `RegisterRoutes` has both `g.DELETE("/:storefront/data", ...)` and `g.POST("/:storefront", ...)` as catch-all parameterised routes — insert the four `/epic/...` lines before `g.GET("/ignored", ...)`, which already sits above both of those.
