...
- `cmd/nexctl/` — REST client binary; `account` (login/logout/whoami/api-key), `profile`, `game` (list/show/add/edit/acquire/rm), `tag` (list/create/rename/rm), `pool` (list/show/create/edit/rm/add/remove/queue/reorder), `sync` (status/connect/disconnect/config/run/review/resolve/skip/retry/reset), and `job` (list/show/retry/cancel) commands. Commands call the user-games/IGDB/tags/pools/sync/jobs REST API via `cliclient`; `game add`/`game edit` are multi-call orchestrations (IGDB search→import→create; ordered platform/hours/status/fields/tags updates). Pools key off `user_game_id` (game refs resolve via `resolveUserGameRef`); `pool queue` bulk-adds then sets the declarative order; `pool create --filter` takes a raw JSON `{"filters":[…]}` string. Sync's storefront slug set is registry-driven via `resolveStorefront` (no hardcoded list); `sync connect` builds per-storefront credential bodies (secrets prompted no-echo); `sync review` is interactive-only (off-TTY use `sync resolve`/`sync skip`); match-review here is the **sync external-games** flow (rematch/skip) — import job-item review lands with the import group. `job`/`sync` confirm destructive ops (cancel/disconnect/reset/skip) via the persistent `-y`
